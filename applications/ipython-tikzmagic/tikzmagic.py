...
class TikzMagics(Magics):
    ...
    def _run_latex(self, code, encoding, dir):
        ...
        # 2021-10-20 srbongers added the following two lines
        if "PATH" in env:
            env["PATH"] = pathsep.join(
                ["/usr/local/bin", env["PATH"], "/Library/TeX/texbin"]
            )
        ...
        try:
            # 2021-10-20 srbongers added '> /dev//null 2>&1' to the line below
            retcode = call(
                "pdflatex --shell-escape tikz.tex > /dev/null 2>&1", shell=True, env=env
            )
            if retcode != 0:
    ...
    def _convert_pdf_to_svg(self, dir):
        ...
        # 2021-10-20 srbongers added the following environment variable 'env' to update the '$PATH'
        env = environ.copy()
        ...
        try:
            # 2021-10-20 srbongers added 'env=env' here
            retcode = call("pdf2svg tikz.pdf tikz.svg", shell=True, env=env)
            if retcode != 0:
    ...
    def _convert_png_to_jpg(self, dir, imagemagick_path):
        ...
        try:
            # 2021-10-20 srbongers added 'env=env' here
            retcode = call(
                "%s tikz.png -quality 100 -background white -flatten tikz.jpg"
                % imagemagick_path,
                shell=True,
                env=env,
            )
